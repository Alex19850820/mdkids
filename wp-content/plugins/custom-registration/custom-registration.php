...
//        $_POST['bio']
        ...
        global $username, $password, $email;
//               $website, $first_name, $last_name, $nickname, $bio;
...
/*parametrs*/	// $website, $first_name, $last_name, $nickname, $bio
//    echo '
//    <style>
//	div {
//		margin-bottom:2px;
...
//	';
    ...
    echo '
        <form action="' . $_SERVER['REQUEST_URI'] . '" method="post" class="modal-reg">
	        <h2>
				Регистрация
			</h2>
...
function registration_validation( $username, $password, $email)  {
   /*, params $website, $first_name, $last_name, $nickname, $bio */
    global $reg_errors;
    $reg_errors = new WP_Error;
 
    if ( empty( $username ) || empty( $password ) || empty( $email ) ) {
        $reg_errors->add('field', 'Не заполнены поля');
    }
	
    if ( strlen( $username ) < 4 ) {
        $reg_errors->add('username_length', 'Логин очень короткий. Минимум 4 символа');
    }
	
    if ( username_exists( $username ) )
        $reg_errors->add('user_name', 'Извините логин уже занят!');
	
    if ( !validate_username( $username ) ) {
        $reg_errors->add('username_invalid', 'Извините, не коректный логин');
    }
    ...
    if ( email_exists( $email ) ) {
        $reg_errors->add('email', 'Этот электронный адрес уже используется');
    }
