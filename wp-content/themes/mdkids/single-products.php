	...
	<section class="product">
		...
		<div class="container m-1">
			...
			<div class="product-info">
				...
				<button data-price="<?=preg_replace('~\D+~','', fw_get_db_post_option($post->ID, 'price_new'))?>" data-id="<?=$post->ID?>" data-user="<?=$current_user->ID?>" class="product-info__btn <?=(!$current_user->ID) ? 'jsReg' : 'add_product'?>">
					Добавить в корзину
				</button>
			</div>
		</div>
